...
export const JobDescriptionInput: React.FC<JobDescriptionInputProps> = ({ onJdChange }) => {
    // Experience State
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center text-lg font-semibold text-slate-700">
        <BriefcaseIcon className="w-6 h-6 mr-2 text-slate-500" />
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <ExperienceIcon className="w-5 h-5 text-slate-500" />
          <h3 className="text-md font-semibold text-slate-700">Experience</h3>
        </div>
        <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-4">
          <form onSubmit={handleAddExperience} className="grid grid-cols-1 sm:grid-cols-5 gap-4">
              <div className="sm:col-span-2">
                <label htmlFor="years-experience" className="block text-sm font-medium text-slate-700 mb-1">Years</label>
                <input type="number" id="years-experience" value={currentYears} onChange={(e) => setCurrentYears(e.target.value)} min="0" className="block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="e.g., 5" />
              </div>
              <div className="sm:col-span-3">
                <label htmlFor="field-experience" className="block text-sm font-medium text-slate-700 mb-1">Field</label>
                <div className="flex gap-2">
                    <input type="text" id="field-experience" value={currentField} onChange={(e) => setCurrentField(e.target.value)} className="block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="e.g., Software Development" />
                    <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-slate-400" disabled={!currentYears || !currentField}>Add</button>
                </div>
              </div>
          </form>
          {experienceItems.length > 0 && <hr className="border-slate-200" />}
          <div className="space-y-2">
            {experienceItems.map(item => (
              <div key={item.id} className="flex items-center justify-between bg-white p-2 rounded-md border border-slate-200">
                  <p className="text-sm text-slate-800"><span className="font-semibold">{item.years} years</span> in {item.field}</p>
                  <button onClick={() => handleRemoveExperience(item.id)} className="text-slate-400 hover:text-red-500"><XCircleIcon className="w-5 h-5" /></button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <EducationIcon className="w-5 h-5 text-slate-500" />
          <h3 className="text-md font-semibold text-slate-700">Education</h3>
        </div>
        <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="education-level" className="block text-sm font-medium text-slate-700 mb-1">Level</label>
              <select id="education-level" value={educationLevel} onChange={(e) => setEducationLevel(e.target.value)} className="block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <option value="">Select level...</option>
                {educationLevels.map(level => <option key={level} value={level}>{level}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="education-field" className="block text-sm font-medium text-slate-700 mb-1">Field of Study</label>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <SkillsIcon className="w-5 h-5 text-slate-500" />
          <h3 className="text-md font-semibold text-slate-700">Skills</h3>
        </div>
        <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
          <div className="flex flex-wrap gap-2">
              {allSkillsToRender.sort().map(skill => {
                ...
                return (
                  <button key={skill} onClick={() => handleSkillToggle(skill)} className={`flex items-center px-3 py-1.5 text-sm font-medium rounded-full transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 ${ isSelected ? 'bg-indigo-600 text-white shadow-md focus:ring-indigo-500' : 'bg-white text-slate-700 border border-slate-300 hover:bg-indigo-50 hover:border-indigo-400 hover:shadow-sm focus:ring-indigo-500' }`}>
          ...
          <form onSubmit={handleAddCustomSkill} className="mt-4 flex items-center gap-2">
            <input type="text" value={customSkill} onChange={(e) => setCustomSkill(e.target.value)} className="flex-grow px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Add a custom skill..." />
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <CertificationIcon className="w-5 h-5 text-slate-500" />
          <h3 className="text-md font-semibold text-slate-700">Certifications</h3>
        </div>
        <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
           <form onSubmit={handleAddCertification} className="flex items-center gap-2">
            <input type="text" value={customCertification} onChange={(e) => setCustomCertification(e.target.value)} className="flex-grow px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="e.g., AWS Certified Cloud Practitioner" />
            <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Add</button>
          </form>
          {certifications.length > 0 && <hr className="my-4 border-slate-200" />}
          <div className="space-y-2">
            {certifications.map((cert, index) => (
              <div key={index} className="flex items-center justify-between bg-white p-2 rounded-md border border-slate-200">
                  <p className="text-sm text-slate-800">{cert}</p>
                  <button onClick={() => handleRemoveCertification(cert)} className="text-slate-400 hover:text-red-500"><XCircleIcon className="w-5 h-5" /></button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <LanguagesIcon className="w-5 h-5 text-slate-500" />
          <h3 className="text-md font-semibold text-slate-700">Languages</h3>
        </div>
        <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
          <div className="flex flex-wrap gap-2">
              {allLanguagesToRender.sort().map(lang => {
                ...
                return (
                  <button key={lang} onClick={() => handleLanguageToggle(lang)} className={`flex items-center px-3 py-1.5 text-sm font-medium rounded-full transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 ${ isSelected ? 'bg-teal-600 text-white shadow-md focus:ring-teal-500' : 'bg-white text-slate-700 border border-slate-300 hover:bg-teal-50 hover:border-teal-400 hover:shadow-sm focus:ring-teal-500' }`}>
                      {isSelected && <CheckIcon className="w-4 h-4 mr-1.5" />}
                      {lang}
                  </button>
                );
          ...
          <form onSubmit={handleAddCustomLanguage} className="mt-4 flex items-center gap-2">
            <input type="text" value={customLanguage} onChange={(e) => setCustomLanguage(e.target.value)} className="flex-grow px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Add a custom language..." />
