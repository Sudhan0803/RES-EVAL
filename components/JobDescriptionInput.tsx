...
export const JobDescriptionUpload: React.FC<JobDescriptionUploadProps> = ({ onFileSelect, selectedFile, isParsing }) => {
    ...
    <div>
      <label className="flex items-center text-lg font-semibold text-slate-700">
       ...
       <p className="text-sm text-slate-500 mt-1 mb-3">
        Select or drop a job description file (PDF or DOCX).
      </p>
      <div
        onClick={handleClick}
        onDrop={handleDrop}
        ...
        aria-label="File upload area for Job Description"
      >
        <div className="space-y-1 text-center">
          ...
          <div className="flex text-sm text-slate-600">
            <span className="relative bg-transparent rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
              ...
              <input ref={fileInputRef} id="jd-file-upload" name="jd-file-upload" type="file" className="sr-only" accept=".pdf,.docx" onChange={handleFileChange} />
            </span>
      ...
      {selectedFile && !isParsing && (
        <div className="mt-4 flex items-center justify-between bg-green-50 text-green-800 p-3 rounded-md border border-green-200">
          ...
          <span className="text-sm font-semibold ml-2 flex-shrink-0">Ready to use</span>
        </div>
      )}
    </div>
  );
